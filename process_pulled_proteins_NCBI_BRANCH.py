...
def get_enzyme(sample_cat):
    if pd.notnull(sample_cat):
        ...
        return None
##############################
spec_info_with_fetch['enzyme'] = spec_info_with_fetch['Biological sample category'].apply(get_enzyme)
spec_info_with_fetch['prot_ident_probab'] = spec_info_with_fetch['Protein identification probability'].str.strip('%').apply(float)
spec_info_with_fetch['pept_ident_probab'] = spec_info_with_fetch['Peptide identification probability'].str.strip('%').apply(float)

#
col_rename = {'Peptide start index':'pept_start',
             'Peptide stop index':'pept_stop',
             'Previous amino acid':'prev_aa',
             'Next amino acid':'next_aa',
             }
#
spec_info_with_fetch.rename(columns=col_rename,inplace=True)
#
cols = [
 'enzyme',
 'prot_ident_probab',
...
#
#
spec_simple = spec_info_with_fetch[cols].drop_duplicates()
#
#
#
#
...
# #################################
spec_simple['uid_fetched'] = spec_simple['fetchid'].apply(lambda fidx: gbrecs[str(int(fidx))].id if pd.notnull(fidx) else None)
spec_simple['GN_fetched']  = spec_simple['fetchid'].apply(lambda fidx: gbrecs[str(int(fidx))].features[1].qualifiers['gene'][0] if pd.notnull(fidx) else None)
...
spec_simple['signal']  = spec_simple['fetchid'].apply( get_signal )
spec_simple['signal_loc']  = spec_simple['fetchid'].apply( get_signal_loc )
spec_simple['tm_span']  = spec_simple['fetchid'].apply( get_tm_span )
...
#
spec_simple['crit_GN'] = (spec_simple['GN']==spec_simple['GN_fetched'])
...
spec_simple['crit_uid_full'] = (spec_simple['uid'].apply(uid_split)==spec_simple['uid_fetched'].apply(uid_split))
spec_simple['crit_uid_maj'] = (spec_simple['uid'].apply(uid_major)==spec_simple['uid_fetched'].apply(uid_major))
...
def pept_isin(row):
    ...
    if pd.notnull(fetchid):
        ...
        None
spec_simple['crit_pept_in'] = spec_simple[['pept','fetchid']].apply(pept_isin,axis=1)
...
def pept_info(row):
    ...
    if pd.notnull(fetchid):
        ...
        None
###############################################################
spec_simple = spec_simple.merge( spec_simple[['pept','fetchid']].apply(pept_info,axis=1),left_index=True,right_index=True )
#
spec_simple['crit_start'] = spec_simple['pept_start'] == spec_simple['start_fetched']
spec_simple['crit_stop'] = spec_simple['pept_stop'] == spec_simple['stop_fetched']
spec_simple['crit_prev_aa'] = spec_simple['prev_aa'] == spec_simple['prev_aa_fetched']
spec_simple['crit_next_aa'] = spec_simple['next_aa'] == spec_simple['next_aa_fetched']
#
#
#
#
crit_cols = [cn for cn in spec_simple.columns if 'crit_' in cn]
# criteria weights ...
...
spec_simple['SCORE'] = spec_simple[crit_cols].fillna(False).mul(crit_weight).sum(axis=1)
#
# ###################################################################
...
# output stuff ...
spec_simple.sort(columns=['pept','SCORE'],inplace=False)[cols+['SCORE']].to_csv('pepts_and_scores.csv',index=False)


spec_simple_sorted = spec_simple.sort(columns=['pept','SCORE'],inplace=False)[cols+['SCORE']]

grouped = spec_simple_sorted.groupby(by='pept')

# grouped['SCORE'].min()
bad_peptides = grouped['SCORE'].max()[grouped['SCORE'].max()<100]



spec_simple_sorted[spec_simple_sorted.pept.isin(bad_peptides.index)].sort(columns=['pept','SCORE'],inplace=False)[cols+['SCORE']].to_csv('pepts_and_scores_BAD.csv',index=False)
